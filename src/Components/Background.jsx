...
const backgroundImgArray = [
    ...
    },
    {
        ...
        locationName: 'Arnold Arboretum of Harvard University, Boston, United States',
...
function Background() {
    let [bgCategory, setBgCategory] = useState('Clear');


 return ( <>
     {backgroundImgArray.map((i) => {
         let {id, path, authorName, authorPath, locationName, locationPath, category} = i;
         if (i.category == bgCategory) {
             return ( <div className="bg bgClear" key={i.id}>
             <section className='bgInfo'>
                <a href={i.authorPath} className="bgAuthorname">Author: {i.authorName}</a>
                <a href={i.locationPath} className="bglocation">Location: {i.locationName}</a> 
                </section>
                </div>
             )
         }
